...
class booked_newspaper(object):
    ...
    def createPage(self):
        ...
        self.button=Button(self.root,text="后退",command=self.goback)
        self.button.pack(side=RIGHT, padx=(0, 20), pady=(0, 20))
        query ="select newspaper.newspaper_id,newspaper.newspaper_name,newspaper.publisher,newspaper.newspaper_price,number from newspaper,book_state where  book_state.user_id = "+str(self.root.user_id) + " and book_state.newspaper_id = newspaper.newspaper_id"
...
class insert(object):
    ...
    def createPage(self):
        ...
        Label(self.page, text='预定份数:').grid(row=2, stick=W, pady=10)
        Entry(self.page, textvariable=self.number).grid(row=2, column=1, stick=E)
    ...
    def insert(self):
        temp_paper_id = self.paper_id.get().split(",")
        #print(temp_paper_id)
        temp_paper_number = self.number.get()
        tag = 0
        if temp_paper_id != "":
            for i in range(len(temp_paper_id)):
                #print(temp_paper_id[i] + "--")
                query = "select * from book_state where  newspaper_id = "+str(temp_paper_id[i])+" and  user_id = "+str(self.root.user_id)
                cursor.execute(query)
                #print (query)
                data = cursor.fetchone()
                #print (data)
                if data == None:
                    query = "insert into book_state VALUES ('%s', '%s',  %s)" % \
                             (temp_paper_id[i], self.root.user_id, temp_paper_number)
                    try:
                    ...
                    #print(str(temp_paper_id[i]))
                    #print(str(temp_paper_number))
                    sql = "UPDATE book_state SET number = number +"+  temp_paper_number +" where  newspaper_id = "+str(temp_paper_id[i])+" and  user_id = "+str(self.root.user_id)
                    print(sql)
                    try:
                        cursor.execute(sql)
                        #messagebox.showinfo(title='提示', message='订阅成功')
